...
//    }
    ...
    override fun onServiceConnected(name: ComponentName, binder: IBinder) {
        sharedViewModel.service = (binder as SerialService.SerialBinder).service
        sharedViewModel.service!!.attach(this)
    ...
    override fun onCreateView(
        ...
        binding.switchLivingRoomLight.setOnClickListener {
            if (binding.switchLivingRoomLight.isChecked) send("e") else send("E")
        }

        binding.switchTvRoomLight.setOnClickListener {
            if (binding.switchTvRoomLight.isChecked) send("d") else send("D")
        }

        binding.switchTvEnergy.setOnClickListener {
            if (binding.switchTvEnergy.isChecked) send("5") else send("6")
        }

        binding.switchKitchenRoomLight.setOnClickListener {
            if (binding.switchKitchenRoomLight.isChecked) send("a") else send("A")
        }

        binding.switchCoffeeEnergy.setOnClickListener {
            if (binding.switchCoffeeEnergy.isChecked) send("c") else send("C")
        }

        binding.switchBlinds.setOnClickListener{
            if (binding.switchCoffeeEnergy.isChecked) send("B") else send("b")
        }
        return view
    }
